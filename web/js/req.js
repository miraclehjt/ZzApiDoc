        ...
        function (data, status) {
            if (status === 'success') {
                if (data.code === 0) {
                    ...
                    var interfaceId = localStorage.getItem("interfaceId");
                    var pid = localStorage.getItem("pid");
                    getProjectList(interfaceId, data.data.id, pid);
                }
            }
            ...
            if (status === 'success') {
                ...
                $.each(data.data, function (n, value) {
                    c += '<tr><td><div class="checkbox"><input type="checkbox" id="checkbox' + n + '" class="styled"><label for="checkbox'
                        ...
                        + value.defValue + '</td><td>' + value.note + '</td><td>' + value.require + '</td><td>'+ value.isGlobal + '</td><td>'
                        + value.createUserName + '</td><td>' + value.createTime + '</td><td><button type="button" class="btn-see-req btn">子参数</button></td></tr>';
                });
                $("#project-list").html(c);
